...
export function validateEmailAddress(value: unknown): string | null {
...
function openWithPowerShell(url: string): Promise<ToolResult> {
  const safe = url.replace(/'/g, "''");
  return new Promise(resolve => {
    exec(
      `powershell.exe -NoProfile -NonInteractive -EncodedCommand ${encodePsScript(
        `Start-Process -FilePath '${safe}'`
      )}`,
...
function openWithCommand(command: string, url: string): Promise<ToolResult> {
...
function openMailClient(url: string): Promise<ToolResult> {
...
registerTool({
  name: 'send_email',
  description:
    'Open a pre-filled email draft in the user\'s default mail client. This does NOT send the message — the user reviews and clicks send themselves. Use for drafting emails when asked.',
  parameters: {
    type: 'object',
    properties: {
      to: {
  ...
  handler: async (params: Record<string, unknown>): Promise<ToolResult> => {
    ...
    const mailtoUrl = buildMailtoUrl(to as string, subject, body);
    ...
    return {
      success: true,
      result: `Opened a pre-filled draft to ${(to as string).trim()} in your mail client — review it there and click send.`,
    };
  },
});
