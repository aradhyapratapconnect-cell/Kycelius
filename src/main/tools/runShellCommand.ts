 ...
 */
function validateOsSyntax(command: string): string | null {
  ...
  if (process.platform === 'win32') {
    const looksUnix =
      UNIX_ONLY_COMMANDS.has(firstTokenLower) ||
  ...
  const looksWindows =
    WINDOWS_ONLY_COMMANDS.has(firstTokenLower) ||
...
registerTool({
  ...
  description:
    "Run a shell command on the user's machine using the OS-native shell " +
  ...
  parameters: {
    ...
    properties: {
      command: {
        type: 'string',
        description:
          'The exact full command text to run, in host-OS shell syntax',
      },
      timeoutSeconds: {
        type: 'number',
        description:
          'Optional hard timeout in seconds (1–300). Defaults to 30. The ' +
  ...
  handler: async (params): Promise<ToolResult> => {
    ...
    const seconds = Math.min(
      Math.max(Number.isFinite(requested) ? requested : DEFAULT_TIMEOUT_SECONDS, 1),
      MAX_TIMEOUT_SECONDS
    );
    ...
    if (outcome.killed || outcome.signal !== null) {
      return {
        success: false,
        error:
          `Command timed out after ${seconds}s and was killed. Partial output:\n${outputBlock}`,
      };
    }

    if (typeof outcome.code === 'string') {
      const reason =
        outcome.code === 'ENOBUFS'
