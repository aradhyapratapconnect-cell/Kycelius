...
import { extractImageMetadata, describeImageMetadata } from './image';

export type ExtractedKind = 'text' | 'note';
...
export interface ExtractedFile {
  ...
  content: string;
  truncated: boolean;
  /** Fatal problems (missing file, unsupported type); `content` carries the reason. */
  failed: boolean;
}
...
export const MAX_FILE_CHARS = 15000;
...
export function parseCsvText(text: string): string {
  ...
  while (i < source.length && rows.length <= MAX_CSV_ROWS) {
    const ch = source[i];
    if (inQuotes) {
      if (ch === '"') {
        if (source[i + 1] === '"') {
  ...
  const lines = rows.slice(0, MAX_CSV_ROWS).map((cells, idx) => {
    return cells.length > 0 ? `Row ${idx + 1}: ${cells.join(' | ')}` : `Row ${idx + 1}: (empty row)`;
  });
  if (truncated) lines.push('[ … remaining rows omitted … ]');
  return lines.join('\n');
}

/** Extraction per file extension; never throws. */
export function extractFile(absPath: string, options?: { maxBytes?: number }): ExtractedFile {
  const name = basenameOf(absPath);
  const fail = (reason: string): ExtractedFile => ({
    name,
    path: absPath,
    kind: 'text',
    content: reason,
  ...
  if (ext === '.xlsx') {
    try {
      const extracted = extractXlsxText(raw);
      if (extracted.length === 0) {
        return fail('no readable cells found in the spreadsheet');
      }
      return { name, path: absPath, kind: 'text', content: extracted, truncated: false, failed: false };
    } catch (err) {
      return fail(err instanceof XlsxParseError ? err.message : `not a valid XLSX: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
  if (IMAGE_EXTS.has(ext)) {
    ...
    return {
      ...
      path: absPath,
      kind: 'note',
      content: describeImageMetadata(meta),
      truncated: false,
      failed: false,
    };
  }
  ...
  return fail(
    `unsupported file type "${ext || '(no extension)'}" — Kyclius can read PDF, XLSX, CSV and plain text (txt/md/json/log) files`
  );
}

function parseCsvOrPlain(text: string, ext: string): string {
  if (ext !== '.csv') return text.replace(/\r\n/g, '\n').replace(/\r/g, '\n').trim();
