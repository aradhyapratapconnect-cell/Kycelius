...
export interface ZipFixtureEntry {
  name: string;
  data: Buffer;
  /** True to store uncompressed (method 0). Default: deflate (method 8). */
  store?: boolean;
}
...
export function buildZip(entries: ZipFixtureEntry[]): Buffer {
  ...
  for (const entry of entries) {
    ...
    const compressed = inflateRaw ? deflateRawSync(entry.data) : entry.data;
...
export function miniXlsx(sharedStrings: string[], sheetXml: string, store = false): Buffer {
