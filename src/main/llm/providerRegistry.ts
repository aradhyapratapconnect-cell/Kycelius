...
export interface ProviderPreset {
  ...
  /** Pre-filled and normally hidden for known presets; required for custom. */
  baseUrl?: string;
  defaultModel: string;
  /** Whether the preset is selectable out of the box (free-tier defaults). */
...
const LEGACY_KEY_CONFIG: Record<string, string> = {
  groq: 'groq_api_key',
  openrouter: 'openrouter_api_key',
};

const LEGACY_MODEL_CONFIG: Record<string, string> = {
  groq: 'groq_model',
...
export function getPreset(presetKey: string, capability: ProviderCapability): ProviderPreset | undefined {
  return [...LLM_PRESETS, ...VOICE_PRESETS].find(
    p => p.presetKey === presetKey && p.capability === capability
  );
}
...
export interface ProviderRow {
  ...
  schema: ProviderSchema;
  baseUrl?: string | null;
  /** User-chosen model/voice name, or the preset default. */
  defaultModel?: string | null;
  enabled: boolean;
  isDefault: boolean;
  /** Whether an API key is currently stored. NEVER includes the key itself. */
  hasKey: boolean;
  createdAt?: string;
  updatedAt?: string;
}
...
function rowToProvider(row: DbProviderRow): ProviderRow {
  return {
    id: row.id,
    capability: row.capability as ProviderCapability,
    presetKey: row.preset_key,
    displayName: row.display_name,
    schema: row.schema as ProviderSchema,
...
function getProviderRowById(id: string): DbProviderRow | undefined {
  return getDb().prepare('SELECT * FROM providers WHERE id = ?').get(id) as DbProviderRow | undefined;
}

function getProviderRowByPreset(
  presetKey: string,
  capability: ProviderCapability
): DbProviderRow | undefined {
  return getDb()
    .prepare('SELECT * FROM providers WHERE preset_key = ? AND capability = ? ORDER BY created_at ASC LIMIT 1')
    .get(presetKey, capability) as DbProviderRow | undefined;
}

function setProviderTimestamps(id: string): void {
  getDb()
    .prepare('UPDATE providers SET updated_at = ? WHERE id = ?')
    .run(new Date().toISOString(), id);
}

export function listProviders(capability?: ProviderCapability): ProviderRow[] {
  const rows = capability
    ? getDb().prepare('SELECT * FROM providers WHERE capability = ? ORDER BY created_at ASC').all(capability)
    : getDb().prepare('SELECT * FROM providers ORDER BY capability ASC, created_at ASC').all();
  return (rows as DbProviderRow[]).map(rowToProvider);
}

export function listPresets(capability?: ProviderCapability): ProviderPreset[] {
  const all = [...LLM_PRESETS, ...VOICE_PRESETS];
...
export function isKnownProvider(id: unknown, capability?: ProviderCapability): id is string {
  if (typeof id !== 'string' || id.length === 0) return false;
  const row = getProvider(id);
  if (row) return capability === undefined || row.capability === capability;
  const preset = [...LLM_PRESETS, ...VOICE_PRESETS].find(p => p.presetKey === id);
  return !!preset && (capability === undefined || preset.capability === capability);
}

export function isKnownLlmProvider(id: unknown): id is ProviderId {
...
export function getDefaultProviderId(capability: ProviderCapability): string {
  const rows = listProviders(capability);
  if (rows.length === 0) {
    return capability === 'llm' ? 'groq' : '';
  }
  for (const row of rows) if (row.isDefault) return row.id;
  for (const row of rows) if (row.enabled) return row.id;
  return rows[0].id;
}
...
export function setDefaultProvider(id: string): void {
  ...
  const capability = row.capability as ProviderCapability;
  getDb()
  ...
  if (capability === 'llm') {
    userConfig.set(LEGACY_DEFAULT_KEY, id);
  }
}
...
export function setProviderEnabled(id: string, enabled: boolean): void {
  getDb().prepare('UPDATE providers SET enabled = ?, updated_at = ? WHERE id = ?').run(enabled ? 1 : 0, new Date().toISOString(), id);
}
...
export function addProvider(input: {
  ...
  const displayName =
    (input.displayName && input.displayName.trim().length > 0 ? input.displayName.trim() : null) ??
  ...
  const defaultModel =
    (input.defaultModel && input.defaultModel.trim().length > 0 ? input.defaultModel.trim() : null) ??
  ...
  if (existing) {
    ...
    getDb()
      .prepare(
        'UPDATE providers SET display_name = ?, base_url = ?, default_model = ?, updated_at = ? WHERE id = ?'
      )
      .run(displayName, baseUrl, defaultModel, now, existing.id);
  } else {
    getDb()
      .prepare(
        `INSERT INTO providers (id, capability, preset_key, display_name, schema, base_url, api_key_encrypted, default_model, enabled, is_default, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, NULL, ?, ?, 0, ?, ?)`
      )
  ...
  const row = getProviderRowById(id);
  if (!row) throw new Error(`Failed to create provider ${id}`);
...
export function updateProvider(
  ...
  const row = getProviderRowById(id);
  if (!row) throw new Error(`Unknown provider row: ${id}`);
...
export function removeProvider(id: string): void {
  ...
  if (legacyKey) userConfig.delete(legacyKey);
  const legacyModel = LEGACY_MODEL_CONFIG[row.preset_key];
  if (legacyModel) userConfig.delete(legacyModel);
  ...
  if (row.is_default === 1) {
    ...
    // without one.
    const fallback = getDefaultProviderId(row.capability as ProviderCapability);
...
function assertEncryptionAvailable(): void {
  ...
  if (!available) {
    throw new LLMProviderError(
      'key_unreadable',
      "Your operating system's secure storage isn't available, so Kyclius can't save an API key safely. Start your system keyring service (e.g. Windows Credential Manager, macOS Keychain, or kwallet/libsecret on Linux) and try again."
    );
  }
}

function getApiKeyBlob(id: string, presetKey: string): string | undefined {
  const row = getProviderRowById(id);
  if (row?.api_key_encrypted) return row.api_key_encrypted;
  // Pre-migration installs: fall back to the legacy user_config mirror.
  const legacy = LEGACY_KEY_CONFIG[presetKey];
  return legacy ? userConfig.get(legacy) : undefined;
}
...
export function hasApiKey(id: string): boolean {
  ...
  return legacy ? !!userConfig.get(legacy) : false;
}

export function removeApiKey(id: string): void {
  getDb().prepare("UPDATE providers SET api_key_encrypted = NULL, enabled = 0, updated_at = ? WHERE id = ?").run(new Date().toISOString(), id);
  const row = getProviderRowById(id);
  const legacy = row ? LEGACY_KEY_CONFIG[row.preset_key] : undefined;
  if (legacy) userConfig.delete(legacy);
}
...
export function getDecryptedApiKey(id: string): string {
  ...
  if (!blob) {
    throw new LLMProviderError(
      'key_not_set',
      `No API key is set for ${label}. Add one in Settings to start using Kyclius.`
    );
  }
  try {
    ...
    throw new LLMProviderError(
      'key_unreadable',
      `Your saved ${label} API key couldn't be decrypted. Re-enter it in Settings.`
    );
  }
}
...
export function setApiKey(id: string, apiKey: string): void {
  ...
  const row = getProviderRowById(id);
  const legacy = row ? LEGACY_KEY_CONFIG[row.preset_key] : undefined;
  if (legacy) userConfig.set(legacy, encrypted);
}
...
export function getConfiguredModel(id: string): string {
  const row = getProviderRowById(id);
  if (row?.default_model) return row.default_model;
  const preset = row ? getPreset(row.preset_key, row.capability as ProviderCapability) : undefined;
  return preset?.defaultModel ?? '';
}

export function setConfiguredModel(id: string, model: string): void {
  ...
  getDb().prepare('UPDATE providers SET default_model = ?, updated_at = ? WHERE id = ?').run(trimmed, new Date().toISOString(), id);
  const row = getProviderRowById(id);
  const legacy = row ? LEGACY_MODEL_CONFIG[row.preset_key] : undefined;
  if (legacy) userConfig.set(legacy, trimmed);
}
...
function seedPresetRows(): void {
  const db = getDb();
  for (const preset of [...LLM_PRESETS, ...VOICE_PRESETS]) {
...
function migrateLegacyConfig(): void {
  ...
  for (const [presetKey, configKey] of Object.entries(LEGACY_KEY_CONFIG)) {
    const legacyKey = userConfig.get(configKey);
  ...
  for (const [presetKey, configKey] of Object.entries(LEGACY_MODEL_CONFIG)) {
    const legacyModel = userConfig.get(configKey);
  ...
  const legacyProvider = userConfig.get(LEGACY_DEFAULT_KEY);
  if (legacyProvider && isKnownLlmProvider(legacyProvider)) {
    getDb().prepare("UPDATE providers SET is_default = 0, updated_at = ? WHERE capability = 'llm'")
      .run(new Date().toISOString());
    getDb().prepare('UPDATE providers SET is_default = 1, updated_at = ? WHERE id = ?')
      .run(new Date().toISOString(), legacyProvider);
  }

  // Guarantee exactly one default; prefer the legacy choice else groq.
  const llmRows = getDb().prepare("SELECT * FROM providers WHERE capability = 'llm'").all() as DbProviderRow[];
...
export function materializeLlmProvider(id: string, build: (config: LLMProviderConfig) => LLMProvider): LLMProvider {
  const row = getProviderRowById(id);
