...
export const DEFAULT_SPEAKER_THRESHOLD = 0.7;
...
/** Encrypted-at-rest persistence for the voiceprint (safeStorage adapter in production). */
export interface SpeakerProfileRepository {
  /** Pure existence check — never touches decryption. */
  hasPrimary(): boolean;
  /** The stored encrypted voiceprint, or null when no profile exists. */
  getPrimaryBlob(): Buffer | null;
  replace(label: string, encryptedVoiceprint: Buffer): void;
  removeAll(): void;
}

/** String<->Buffer encryption boundary (Electron safeStorage adapter in production). */
export interface SpeakerCrypto {
  encrypt(plainText: string): Buffer;
  decrypt(blob: Buffer): string;
}

export interface SpeakerVerificationDeps {
  embed(pcm: Float32Array): Promise<number[]>;
  repository: SpeakerProfileRepository;
  crypto: SpeakerCrypto;
}

export interface SpeakerVerifyResult {
  matched: boolean;
  /** Raw cosine similarity against the stored voiceprint (0 when bypassed). */
  confidence: number;
  mode: 'gated' | 'bypassed';
  reason?:
    | 'disabled'
...
class SpeakerVerificationService {
  private deps: SpeakerVerificationDeps | null = null;
  private enabled = false;
  private threshold = DEFAULT_SPEAKER_THRESHOLD;

  configure(deps: SpeakerVerificationDeps, options?: { threshold?: number }): void {
    this.deps = deps;
   ...
   */
  async enroll(audioSamples: Float32Array[]): Promise<EnrollResult> {
    ...
    if (
      !Array.isArray(audioSamples) ||
      audioSamples.length < MIN_ENROLLMENT_SAMPLES ||
      audioSamples.length > MAX_ENROLLMENT_SAMPLES
    ) {
      return {
        ok: false,
        error: `Enrollment needs ${MIN_ENROLLMENT_SAMPLES}-${MAX_ENROLLMENT_SAMPLES} short phrases.`,
      };
    }
    ...
    try {
      ...
      const averaged = l2Normalize(
        embeddings[0].map((_, d) => embeddings.reduce((sum, vec) => sum + vec[d], 0))
      );

      const enrolledAt = new Date().toISOString();
      deps.repository.replace(
        PRIMARY_LABEL,
        deps.crypto.encrypt(JSON.stringify({ embedding: averaged }))
      );
   ...
   */
  async verify(audioSample: Float32Array): Promise<SpeakerVerifyResult> {
    if (!this.enabled) {
    ...
    try {
      ...
      const parsed = JSON.parse(deps.crypto.decrypt(blob)) as { embedding?: number[] };
      if (!Array.isArray(parsed.embedding)) throw new Error('voiceprint payload malformed');
