...
export interface WakeVoiceError {
  code: string;
  message: string;
}
...
export interface WakeWordDeps {
  ...
  onStateChanged?(state: WakeListeningState): void;
  onError?(error: WakeVoiceError): void;
  isCommandSessionActive?(): boolean;
  isSpeaking?(): boolean;
}
...
export interface WakeWordOptions {
  ...
  silenceMsToDetect?: number;
  detectWindowSeconds?: number;
  maxBufferSeconds?: number;
  cooldownMs?: number;
  minDetectIntervalMs?: number;
  errorRepeatMs?: number;
}
...
export const DEFAULT_WAKE_PHRASE = 'Hey Kyclius';
...
export function validateWakePhrase(phrase: unknown): WakePhraseValidation {
  ...
  if (text.length < 4) {
    warnings.push(
      'This phrase is very short and may trigger accidentally. A longer phrase like "Hey Kyclius" works better.'
    );
  }
  ...
  if (normalizedTokens.length === 1 && COMMON_SINGLE_WORDS.has(normalizedTokens[0])) {
    warnings.push(
      'This is a very common word and will likely trigger often during normal conversation. Try a more distinctive phrase.'
    );
  }
 ...
 */
export function transcriptContainsPhrase(transcript: unknown, phrase: string): boolean {
  ...
  if (spokenWords.length === 0 || phraseWords.length === 0) return false;

  for (let start = 0; start <= spokenWords.length - phraseWords.length; start++) {
    let matched = 0;
    for (let i = 0; i < phraseWords.length; i++) {
      if (tokensClose(spokenWords[start + i], phraseWords[i])) matched++;
    }
    ...
    if (matched >= phraseWords.length - allowedMisses) return true;
  }
  return false;
}
...
interface ResolvedWakeWordService {
  configure(deps: WakeWordDeps, options?: WakeWordOptions): void;
  setEnabled(enabled: boolean): void;
  isEnabled(): boolean;
  getState(): WakeListeningState;
  getBufferedSampleCount(): number;
  ingestChunk(chunk: Float32Array): void;
  handleAmbientCaptureStarted(): void;
  handleCaptureFailed(error: WakeVoiceError): void;
  reset(): void;
}

class WakeWordService implements ResolvedWakeWordService {
  private deps: WakeWordDeps | null = null;
  private options = { ...DEFAULT_OPTIONS };

  private enabled = false;
  private state: WakeListeningState = 'off';

  private buffer: Float32Array[] = [];
  private bufferedSamples = 0;

  private sawSpeechSinceLastDetect = false;
  ...
  ingestChunk(chunk: Float32Array): void {
    if (!this.enabled || this.state === 'unavailable' || chunk.length === 0) return;
  ...
  private clearSpeechTracking(): void {
    this.sawSpeechSinceLastDetect = false;
    this.lastLoudAt = 0;
  }

  private appendChunk(chunk: Float32Array): void {
  ...
  private takeDetectionWindow(): Float32Array {
    const maxWindowSamples =
      Math.min(this.options.detectWindowSeconds, this.options.maxBufferSeconds) *
  ...
  private async runDetection(): Promise<void> {
    ...
    if (!deps || this.detecting || !this.enabled) return;
  ...
  private scheduleRevertToListening(): void {
  ...
  private emitThrottledError(err: unknown): void {
  ...
  private toWakeError(err: unknown): WakeVoiceError {
    const name = err instanceof Error ? err.name : '';
    if (name === 'WhisperNotConfiguredError') {
      return {
        code: 'stt_engine_unavailable',
        message:
          'Wake word listening needs a local speech engine. Configure whisper.cpp in Settings, or turn off background listening.',
      };
    }
    const message = err instanceof Error ? err.message : String(err);
  ...
  private emitError(error: WakeVoiceError): void {
    this.deps?.onError?.(error);
  }

  private setState(state: WakeListeningState): void {
