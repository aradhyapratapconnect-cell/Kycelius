...
import { memoryFacts, type MemoryFact } from '../db/db';

export type FactSource = 'auto_learned' | 'user_added';
...
export function getFact(key: string): MemoryFact | undefined {
  ...
  return memoryFacts.getByKey(normalized);
}
...
export function setFact(
  ...
  return memoryFacts.set(normalizedKey, normalizedValue, source);
}

export function listFacts(): MemoryFact[] {
  return memoryFacts.getAll();
}
...
function normalizeKey(key: unknown): string | null {
  ...
  if (cleaned.length === 0 || cleaned.length > KEY_MAX_LENGTH) return null;
  return cleaned;
}
...
function normalizeValue(value: unknown): string | null {
  ...
  if (cleaned.length === 0 || cleaned.length > VALUE_MAX_LENGTH) return null;
  return cleaned;
}
 ...
 */
export function getRelevantFacts(userText: string, limit = RELEVANT_FACT_LIMIT): MemoryFact[] {
...
export function buildMemoryContextBlock(facts: MemoryFact[]): string | null {
  if (facts.length === 0) return null;
  const lines = facts.map(fact => `- ${fact.key}: ${fact.value}`);
  return (
    'Durable facts you already know about the user (use naturally when relevant):\n' +
    lines.join('\n')
  );
}

// ---------------------------------------------------------------------------
// Auto-learning: detect fact statements/corrections in the user's message
// ---------------------------------------------------------------------------
...
export function extractFactCandidates(userText: string): FactCandidate[] {
  ...
  for (const pattern of FACT_PATTERNS) {
    ...
    if (cleanKey && cleanValue) candidates.push({ key: cleanKey, value: cleanValue });
  }
