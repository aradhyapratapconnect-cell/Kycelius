...
exports.CronExpressionError = void 0;
exports.parseCron = parseCron;
exports.nextFireAt = nextFireAt;
...
class CronExpressionError extends Error {
    expression;
...
exports.CronExpressionError = CronExpressionError;
function parseField(field, min, max) {
    ...
    for (const part of field.split(',')) {
        ...
        if (rangeSpec === '*') {
            ...
            if (!range)
                throw new Error(`bad field segment "${part}"`);
            lo = Number(range[1]);
            hi = Number(range[2]);
        }
        if (lo < min || hi > max) {
        ...
        if (lo > hi)
            throw new Error(`reversed range in "${part}"`);
...
function parseCron(expression) {
    ...
    if (fields.length !== 5) {
        throw new CronExpressionError('expected exactly 5 fields: minute hour day-of-month month day-of-week', expression);
    }
    try {
        ...
        if (err instanceof CronExpressionError)
            throw err;
 ...
 */
function nextFireAt(expression, from = new Date()) {
    ...
    throw new CronExpressionError('no fire time exists within 6 years — the expression is effectively unsatisfiable', expression);
}
function dayMatches(cron, dayOfMonth, dayOfWeek) {
    ...
    if (dom.all && dow.all)
        return true;
    if (dom.all)
        return dow.values.has(dayOfWeek);
    if (dow.all)
        return dom.values.has(dayOfMonth);
...
function describeCron(expression) {
    ...
    if (fields.length !== 5)
        return expression;
    const restIsEveryDay = fields[1] === '*' && fields[2] === '*' && fields[3] === '*' && fields[4] === '*';
