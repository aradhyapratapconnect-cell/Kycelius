...
exports.GithubToolError = void 0;
exports.hasGithubToken = hasGithubToken;
exports.encryptAndStoreGithubToken = encryptAndStoreGithubToken;
exports.removeGithubToken = removeGithubToken;
...
exports.GithubToolError = GithubToolError;
function assertEncryptionAvailable() {
    ...
    if (!available) {
        throw new GithubToolError('encryption_unavailable', "Your operating system's secure storage isn't available, so Kyclius can't save a GitHub token safely. Start your system keyring service (Windows Credential Manager, macOS Keychain, or kwallet/libsecret on Linux) and try again.");
    }
}
function hasGithubToken() {
    return !!db_1.userConfig.get(TOKEN_CONFIG_KEY);
}
/** Encrypts via safeStorage before writing; plain text never touches SQLite. */
function encryptAndStoreGithubToken(token) {
    assertEncryptionAvailable();
    const encrypted = electron_1.safeStorage.encryptString(token).toString('base64');
    db_1.userConfig.set(TOKEN_CONFIG_KEY, encrypted);
}
function removeGithubToken() {
    db_1.userConfig.delete(TOKEN_CONFIG_KEY);
}
function resolveGithubToken() {
    if (!hasGithubToken()) {
        return {
            ok: false,
            error: 'No GitHub token is set. Add a Personal Access Token in Settings so Kyclius can talk to GitHub on your behalf.',
        };
    }
    const encrypted = db_1.userConfig.get(TOKEN_CONFIG_KEY);
    try {
        return { ok: true, token: electron_1.safeStorage.decryptString(Buffer.from(encrypted, 'base64')) };
    }
    catch {
        return {
            ok: false,
            error: "Your saved GitHub token couldn't be decrypted. Re-enter it in Settings.",
        };
    }
}
async function githubApiRequest(method, path, token, body) {
    ...
    try {
        ...
        throw new GithubToolError('network', "Couldn't reach GitHub (network failure or timeout). Check your internet connection and try again.");
    }
    finally {
        clearTimeout(timer);
    }
    if (response.status === 401) {
        throw new GithubToolError('invalid_token', 'GitHub rejected the stored token as invalid or expired (401). Generate a new Personal Access Token and update it in Settings.');
    }
    if (response.status === 403) {
        if (response.headers.get('x-ratelimit-remaining') === '0') {
            const resetEpoch = Number(response.headers.get('x-ratelimit-reset') ?? 0);
            const resetAt = resetEpoch > 0 ? new Date(resetEpoch * 1000).toLocaleTimeString() : 'soon';
            throw new GithubToolError('rate_limited', `GitHub API rate limit reached for your token. The limit resets at ${resetAt}; until then GitHub requests can't go through.`);
        }
        throw new GithubToolError('forbidden', 'GitHub refused this action (403). Your token may be missing the required scope (read needs `repo`; commenting needs `repo` too).');
    }
    ...
    if (!response.ok) {
        ...
        throw new GithubToolError('unexpected', `GitHub returned an unexpected error: ${detail}`);
    }
    return { status: response.status, headers: response.headers, json };
}
/** Turns a 404 into the right "repo" vs "issue" message for the context. */
function notFoundMessage(context) {
...
function runGithubTool(fn) {
    return fn().catch((err) => err instanceof GithubToolError
...
(0, toolRegistry_1.registerTool)({
    name: 'github_read',
    description: 'List and summarize open issues or pull requests for a GitHub repository ' +
    ...
    handler: async (params) => runGithubTool(async () => {
        ...
        const items = (Array.isArray(data.json) ? data.json : []);
        const filtered = kind === 'issues' ? items.filter(item => !item.pull_request) : items;
...
(0, toolRegistry_1.registerTool)({
    name: 'github_write',
    description: 'Post a comment on a GitHub issue or pull request. Always asks the user ' +
    ...
    handler: async (params) => runGithubTool(async () => {
        ...
        try {
            ...
            if (err instanceof GithubToolError && err.kind === 'not_found') {
                throw new GithubToolError('not_found', notFoundMessage({ kind: 'issue', owner, repo, number }));
            }
            throw err;
        }
        const url = data.json?.html_url;
        return {
            success: true,
            result: url
