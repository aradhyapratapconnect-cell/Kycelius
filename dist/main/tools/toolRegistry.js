...
exports.DEFAULT_TOOL_TIMEOUT_MS = void 0;
exports.setExecutionGate = setExecutionGate;
exports.setToolExecutionNotifier = setToolExecutionNotifier;
...
function setExecutionGate(gate) {
    executionGate = gate;
}
let executionNotifier = null;
function setToolExecutionNotifier(notifier) {
...
function registerTool(definition) {
    ...
    if (!VALID_TIERS.includes(definition.permissionTier)) {
        throw new Error(`Tool "${definition.name}" must declare an explicit permissionTier of 'auto' or 'confirm_required' — no default tier is allowed`);
    }
...
function recordStatus(id, status, result) {
    try {
        db_1.toolExecutions.updateStatus(id, status, result);
    }
    catch {
    ...
    if (executionNotifier) {
        try {
            ...
            console.error(`[toolRegistry] Execution notifier threw for ${id}: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
}
...
async function executeToolCall(toolName, rawParams, options = {}) {
    const timeoutMs = typeof options.timeoutMs === 'number' && options.timeoutMs > 0
        ? options.timeoutMs
        : exports.DEFAULT_TOOL_TIMEOUT_MS;
    ...
    const prepareParams = (raw) => {
        let candidate = raw;
        if (tool.normalizeParams) {
            const normalized = tool.normalizeParams(candidate);
        ...
        if (tool.validate) {
            ...
            if (semanticError)
                return { valid: false, error: semanticError };
        }
