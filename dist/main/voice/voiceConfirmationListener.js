...
exports.voiceConfirmationListener = exports.DEFAULT_CONFIRMATION_WINDOW_MS = void 0;
exports.classifySpeech = classifySpeech;
exports.describePendingAction = describePendingAction;
...
exports.DEFAULT_CONFIRMATION_WINDOW_MS = 15000;
...
function classifySpeech(raw) {
    ...
    for (const affirmative of AFFIRMATIVE_WORDS) {
        if (wordSet.has(affirmative))
            return 'approve';
    }
    return 'ignore';
}
function describePendingAction(confirmation) {
    let paramsText;
    try {
...
class VoiceConfirmationListener {
    ...
    getWindowConfirmationId() {
        return this.windowState?.id ?? null;
    }
    handleQueueChange(pending) {
    ...
    openWindow(confirmation) {
        ...
        if (!deps || this.windowState)
            return;
        if (deps.isEnabled && !deps.isEnabled())
            return;
        const timeoutMs = deps.timeoutMs ?? exports.DEFAULT_CONFIRMATION_WINDOW_MS;
        const startedOwnCapture = !deps.isCapturing();
        const state = {
            id: confirmation.id,
            timer: setTimeout(() => {
                if (this.windowState !== state || state.settled)
                    return;
                state.settled = true;
        ...
        state.unsubscribeTranscripts = deps.subscribeTranscripts((text, isFinal) => {
            ...
            if (isFinal === false)
                return;
            if (this.windowState !== state || !state.gateOpen || state.settled)
                return;
...
exports.voiceConfirmationListener = new VoiceConfirmationListener();
