...
exports.parseCmuDict = parseCmuDict;
exports.loadLexicon = loadLexicon;
exports.resetLexiconCacheForTests = resetLexiconCacheForTests;
...
async function loadLexicon(lexiconPath) {
    if (cachedLexicon && cachedPath === lexiconPath)
        return cachedLexicon;
    try {
        const raw = await (0, promises_1.readFile)(lexiconPath, 'utf-8');
...
function resetLexiconCacheForTests() {
    cachedLexicon = null;
    cachedPath = '';
}
// ---------------------------------------------------------------------------
...
function arpabetToIpa(arpabet) {
    ...
    for (let i = 0; i < phones.length; i++) {
        ...
        if (isVowelBase(base)) {
            ...
            if (base === 'AH' && stress === '0')
                out += `${mark}ə`;
            else if (base === 'ER' && stress === '0')
                out += `${mark}ɚ`;
            else
                out += `${mark}${VOWEL_IPA[base]}`;
            continue;
        }
...
function cardinalUnderThousand(n) {
    if (n < 20)
        return ONES[n];
    if (n < 100) {
...
function cardinal(n) {
    ...
    if (n < 1000)
        return cardinalUnderThousand(n);
    if (n < 1_000_000) {
...
function ordinal(token) {
    const lower = token.toLowerCase();
    if (ORDINAL_SPECIAL[lower])
        return ORDINAL_SPECIAL[lower];
...
function money(amount, one, many, smallOne, smallMany) {
    ...
    if (cents !== undefined) {
        ...
        if (c > 0)
            out += ` and ${cardinal(c)} ${c === 1 ? smallOne : smallMany}`;
    }
    return out;
}
...
function expandNumbers(text) {
    let out = text;
    out = out.replace(/\$(\d[\d,]*)(?:\.(\d{1,2}))?/g, (_m, d, c) => money(`${d}${c !== undefined ? `.${c}` : ''}`, 'dollar', 'dollars', 'cent', 'cents'));
    out = out.replace(/£(\d[\d,]*)(?:\.(\d{1,2}))?/g, (_m, d, c) => money(`${d}${c !== undefined ? `.${c}` : ''}`, 'pound', 'pounds', 'penny', 'pence'));
    out = out.replace(/€(\d[\d,]*)(?:\.(\d{1,2}))?/g, (_m, d, c) => money(`${d}${c !== undefined ? `.${c}` : ''}`, 'euro', 'euros', 'cent', 'cents'));
    out = out.replace(/\b(\d[\d,]*\.?\d*)\s?%/g, (_m, n) => `${plainInteger(n)} percent`);
    out = out.replace(/\b(\d+)(st|nd|rd|th)\b/gi, m => ordinal(m));
    out = out.replace(/\b(\d[\d,]*)\.(\d+)\b/g, (_m, a, b) => `${plainInteger(a)} point ${spellDigits(b)}`);
...
function pluralSibilant(stemPhones) {
...
function lookupWord(word, lexicon) {
    ...
    if (lexicon.has(upper))
        return lexicon.get(upper) ?? null;
    // Dotted abbreviations ("mr." -> "mr", "u.s.a" -> "usa").
    const stripped = upper.replace(/\./g, '');
    if (stripped !== upper && lexicon.has(stripped))
        return lexicon.get(stripped) ?? null;
    return null;
}
/**
...
function isKeptPunct(ch) {
    return ch === '.' || ch === ',' || ch === '!' || ch === '?' || ch === ';' || ch === ':';
}
function splitAffixes(token) {
    let start = 0;
    let end = token.length;
    while (start < end && !/[a-z0-9'’-]/.test(token[start]))
        start++;
    while (end > start && !/[a-z0-9'’-]/.test(token[end - 1]))
        end--;
    return {
 ...
 */
function phonemize(text, lexicon) {
